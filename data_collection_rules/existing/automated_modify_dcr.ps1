...
function Invoke-DCRModify {
    ...
    End {
        ...
        if ($DCR_Action.ToLower() -eq 'get') {
            ...
            Write-Host "Upon completion, you can run the CmdLet DCR-Modify with the `"-DCR_Action Set`" option." -ForegroundColor Yellow
        }else{
            Write-Host "Action Selected: `"$DCR_Action`"" -ForegroundColor Red
            Write-Host "Your DCR: $DCRName.json, is now ready to be sent via Azure REST API!" -ForegroundColor Yellow
